...
# Algorithm: read_csv function reads a CSV file containing song data. It creates a dictionary where keys are artists and
# values are lists of songs associated with each artist. The process_data function processes the data from the
# dictionary of songs to determine the count of songs by each artist. The find_most_common_word function finds
# the most common word among all songs in the dictionary of songs along with its frequency.
...
def process_data(songs_dict):
    ...
    Returns: A list of tuples containing the count of songs, the corresponding artist name,
     and the number of songs by that artist, sorted in descending order of song count.
    """
...
def main(filename):
    ...
    print(
        " a. For songs with duplicate words, I used a dictionary to count the occurrences of words in the song titles.")
    print(
        "  b. For problem 3, I iterated through all song titles,"
        " counted the occurrences of words with more than 3 letters, and found the most common word.")
