...
def process_data(songs_dict):
    ...
    Returns: A list of tuples containing the count of songs and the corresponding artist name, sorted in descending order of song count.
    """
    # Initialize an empty list to store tuples of song count and artist name
    song_count_by_artist = [(len(songs), artist) for artist, songs in songs_dict.items()]
...
def main(filename):
    ...
    top_five_artists = [artist for _, artist in process_data(songs_dict)[:5]]
    common_word, common_word_count = find_most_common_word(songs_dict)
    print(f"1. Top artist: {top_artist}")
    print(f"2. Top five artists: {top_five_artists}")
